...
def scroll_randomly():
    # Scroll downward to a random pixel
    scroll_down = random.randint(500, 1500)
    driver.execute_script(f"window.scrollBy(0, {scroll_down});")
    time.sleep(random.uniform(1, 3))  # Random delay after scrolling down

    # Scroll a bit upward
    scroll_up = random.randint(100, 300)
    driver.execute_script(f"window.scrollBy(0, {-scroll_up});")
    time.sleep(random.uniform(1, 3))  # Random delay after scrolling up

    # Pause for a random second
    time.sleep(random.uniform(1, 3))

    # Scroll upward
    scroll_up = random.randint(500, 1500)
    driver.execute_script(f"window.scrollBy(0, {-scroll_up});")
    time.sleep(random.uniform(1, 3))  # Random delay after scrolling up
